...
from evals.action_anticipation_frozen.epickitchens import (
    filter_annotations as ek100_filter_annotations_video,
)
from evals.action_anticipation_frozen.epickitchens import (
    make_webvid as ek100_make_webvid_video,
)
from evals.action_anticipation_frozen.epickitchens_frames import (
    filter_annotations as ek100_filter_annotations_frames,
)
from evals.action_anticipation_frozen.epickitchens_frames import (
    make_webvid as ek100_make_webvid_frames,
)
...
def init_data(
    ...
    del drop_last  # unused; keep for compatibility
    ...
    if "ek100" in dataset.lower():
        if input_format.lower() == "frames":
            make_webvid = ek100_make_webvid_frames
        elif input_format.lower() == "video":
            make_webvid = ek100_make_webvid_video
        else:
            raise ValueError(f"Unsupported EK input_format={input_format!r}. Use 'video' or 'frames'.")

    if make_webvid is None:
        raise ValueError(f"Unsupported dataset={dataset!r} for action_anticipation_frozen dataloader.")
...
def filter_annotations(
    ...
    **kwargs,
    ...
    if "ek100" in dataset.lower():
        if input_format.lower() == "frames":
            _filter = ek100_filter_annotations_frames
        elif input_format.lower() == "video":
            _filter = ek100_filter_annotations_video
        else:
            raise ValueError(f"Unsupported EK input_format={input_format!r}. Use 'video' or 'frames'.")

    if _filter is None:
        raise ValueError(f"Unsupported dataset={dataset!r} for action_anticipation_frozen filter_annotations.")
